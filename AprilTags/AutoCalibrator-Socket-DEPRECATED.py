...
# UNTESTED
# Listens on the ctrl socket for READY, then pushes commands to the loop socket
# Once successful it waits for the next READY command on the ctrl socket

ctrl_socket_path = "/tmp/camera_ctrl.sock"
loop_socket_path = "/tmp/camera_loop.sock"
...
class AutoCalibrator:
    def __init__(
        ...
        self.loop_socket_path = loop_socket_path
        self.loop = socket.socket(socket.AF_UNIX, socket.SOCK_DGRAM)
        self.loop.connect(self.loop_socket_path)
...
def ctrl_socket_thread():
    global ready, tagID
    while True:
        try:
            if not ready:
                server_sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
                server_sock.connect(ctrl_socket_path)
                ...
                if data.decode().strip() == "READY":
                    print("Camera control socket ready")
                    ready = True
...
def main() -> None:
    threading.Thread(target=ctrl_socket_thread).start()
    while True:
        if not ready:
            time.sleep(0.1)
            continue
        calibrator = AutoCalibrator(camera_index=0)
        calibrator.run()
